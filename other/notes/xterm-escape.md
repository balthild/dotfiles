...
For example, `<CSI>1;3C` means <kbd>Alt+Right</kbd> (the `1` is a dummy parameter), but `<CSI>3C` means just <kbd>Right</kbd> (the `3` becomes a parameter being ignored).
...
Similar to the CSI-27 extensions, but looks like cursor control sequences and supports Unicode. The first parameter is the Unicode codepoint and the second parameter is the encoded modifiers.
